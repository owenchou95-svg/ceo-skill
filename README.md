...
CEO 最值得看的不是它会“写 prompt”，而是它会先判断能不能执行：

| 用户输入 | CEO 路线 | 交付物 |
| --- | --- | --- |
| “给 README 增加安装步骤” | `Task Mode -> Direct Path` | 带 inventory 证据和验证命令的执行 brief |
| “我想把 CEO skill 做得更智能” | `Goal Mode -> Incomplete` | 一个命中缺口的 `Blocking Question`，不生成执行 prompt |
| “我想清理生产数据库并重新部署系统” | `Goal Mode -> Risk Boundary` | 风险边界澄清，不给删除/部署命令 |
...
| 目标不够清楚 | `Goal Mode: Incomplete` | 缺少 desired state / acceptance / first slice 时只问一个 blocking question | [evals/live-model/cases.jsonl](evals/live-model/cases.jsonl) |
| 高风险目标 | `Goal Mode: Risk Boundary` | 生产、权限、删除、部署目标先收集授权、回滚、dry-run、stop conditions | [evals/live-model/cases.jsonl](evals/live-model/cases.jsonl) |

结果卡片：

- [assets/result-card.md](assets/result-card.md)：记录本轮打磨后的分数、鲁班体检和验证证据。

输出会先包含 `Mode Router`，判断当前输入走 `Task Mode` 还是 `Goal Mode`。`Task Mode` 继续生成可执行 brief；`Goal Mode` 先生成目标合同、缺口问题、发现澄清或风险边界澄清，只在首个可执行切片足够清楚时再回到 Task Mode。

Task Mode 响应会包含这些固定段落：
...
Goal Mode 响应会包含 `Goal Mode`、`Goal Contract Check`、`Clarification Type`、`Inventory Decision`、`Route Decision`，并按状态输出 `Blocking Question`、`Task Mode Handoff`、`Final Prompt` 或 `Clarification Prompt`。
...
| 能力 | 交付物 | 为什么重要 |
| --- | --- | --- |
| 模式路由 | `Mode Router` | 先区分可执行任务和需要目标细化的请求 |
| 需求分流 | `Direct Path` / `Clarification Path` | 在 Task Mode 内防止模糊或高风险任务被直接执行 |
| 目标细化 | `Goal Mode` / `Goal Contract Check` / `Domain Gate` | 把目标型请求收敛成缺口问题、发现澄清、风险边界或首个可执行切片 |
| Inventory 决策 | `Inventory Decision` | 显式说明是否运行 inventory，以及输入是原始任务、澄清意图还是首个切片 |
